...
class Player(pygame.sprite.Sprite):
    def __init__(self, level):
        super().__init__()
        self.player_anim_list = Player.create_anim_list(
            ['Stand/player_stand', 'Run/player_run', 'Jump/player_jump', 'Fall/player_fall'])
        self.image = self.player_anim_list[0][0]
        self.rect = self.image.get_rect(
            bottomleft=(level.start_x, level.start_y))
        self.level = level
        self.anim_index = 0
    ...
    def create_anim_list(name_list):
        ...
        for name in name_list:
            ...
            anim_list = []
            while i < 5:
                anim_list.append(pygame.image.load(
                    f'Images/Player/{name}_{i}.png'))
                i += 1
    ...
    def flip_image_list(self, player_image_list):
        i = 0
        while i < len(player_image_list):
            o = 0
            while o < len(player_image_list[i]):
                player_image_list[i][o] = pygame.transform.flip(
                    player_image_list[i][o], True, False)
                o += 1
            i += 1
    ...
    def input(self):
        ...
        if key[pygame.K_d]:
            ...
            if self.face_side == 'l':
                self.face_side = 'r'
                self.player_anim_list = self.flip_image_list(
                    self.player_anim_list)
        elif key[pygame.K_a]:
            self.rect.x -= 6
            if self.anime_list_index < 2:
                self.anime_list_index = 1
    ...
    def next_scene_right(self, scene_list):
        if self.rect.right-self.rect.width/2 > 1400:
    ...
    def next_scene_left(self, scene_list):
        if self.rect.right-self.rect.width/2 < 0:
